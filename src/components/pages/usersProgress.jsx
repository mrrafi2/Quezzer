...
import classes from "../style/userProgress.module.css";

export default function UserCategoryProgress() {
  const { uid } = useParams(); 
  ...
  const maxCategoryScore = 9600; // 32 labels * 300 (example maximum per category)
  ...
  useEffect(() => {
    ...
    // Fetch badge data for category scores
  ...
  return (
    ...
    >
      ...
      <h5>
        {userData?.displayName || (userData?.email ? userData.email.split("@")[0] : "User")}
      </h5>
      <br />
      <p style={{textAlign:'center',fontSize:'1.1rem', fontWeight:600}}>Category Progress</p>
